...
class RecallServer extends Server {
  ...
  private setupMiddleware(): void {
    ...
    this.app.use(
      cors({
        origin: [
          'http://localhost:3000',
          'https://jessamarie.github.io/recall/',
          'https://jessamarie.github.io'
        ], // * (wildcard) permits any origin
  ...
  private connectDB(): void {
    mongoose.connect(this.mongoUrl).then(
      (conn: Mongoose) => {
        Logger.imp(
          `Database ${conn.connections[0].name} is ready on ${conn.connections[0].host}`
        );
      },
      (err) => {
        Logger.err(err);
